from utils import action_type, action_matching
...
def _decode_image(example, image_height, image_width, image_channels):
...
def parse_example(ex, get_images, get_annotations, get_actions):
    ...
    output_ep = {"goal": goal, "step_id": step_id}
    ...
    if get_annotations:
        flattened_positions = np.array(ex.features.feature['image/ui_annotations_positions'].float_list.value)
        ui_text = [value.decode('utf-8') for value in ex.features.feature['image/ui_annotations_text'].bytes_list.value]
        ui_type = [value.decode('utf-8') for value in ex.features.feature['image/ui_annotations_ui_types'].bytes_list.value]
    ...
    return ep_id, output_ep

def parse_episode(episode, curr_split, output_dir, dataset_name, get_images, get_annotations, get_actions):
    parsed_episode = []
    id = str(uuid.uuid4())
    
    for ex in episode:
        ep_id, output_ep = parse_example(ex, get_images, get_annotations, get_actions)
        if get_images:
            image = output_ep.pop('image')
            image = image.numpy()
            image = Image.fromarray(image).convert('RGB')
            img_folder_path = os.path.join(output_dir, curr_split)
            os.makedirs(img_folder_path, exist_ok=True)
            if dataset_name == "single":
                ep_id = id
            img_path = ep_id + '_' + str(output_ep["step_id"]) + '.jpeg'
...
def fetch_episode(dataset_name, output_dir, data_split, get_images, get_annotations, get_actions):
    ...
    all_parsed_episode = {"train": [], "val": [], "test": []}
    total_screens = {"train": 0, "val": 0, "test": 0}
    ...
    for idx, d in enumerate(tqdm(dataset)):
        ...
        if episode_id is None:
            ...
            try:
                output = parse_episode(episode, curr_split, output_dir, dataset_name, get_images, get_annotations, get_actions)
        ...
        if idx>500000 and dataset_name == "google_apps":
            break
...
if __name__ == '__main__':
    ...
    split_file = f"dataset/{args.dataset}_texts_splits.json"
    output_dir = f"dataset/aitw/{args.dataset}"
    all_parsed_episode = fetch_episode(args.dataset, output_dir, split_file, args.get_images, args.get_annotations, args.get_actions)
    output_dir = output_dir + '/' + args.dataset
    with open(f"{output_dir}_train.obj", "wb") as wp:
        pickle.dump(all_parsed_episode["train"], wp)
    with open(f"{output_dir}_val.obj", "wb") as wp:
        pickle.dump(all_parsed_episode["val"], wp)
    with open(f"{output_dir}_test.obj", "wb") as wp:
